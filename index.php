...
function talforweb_review_link()
{
    ...
    if($_REQUEST['settings-updated'] == 'true'){
        add_action('admin_notices', 'talforweb_print_errors');
    }
...
function talforweb_variable_setting()
{
    ...
    register_setting('talforweb-review-settings-group', 'talforweb_write_review_url');
    register_setting('talforweb-review-settings-group', 'talforweb_show_review');
}

function talforweb_print_errors(){
     ?>
    <div class="notice notice-success is-dismissible"><p>Setting saved</p></div>
    <?php
}
...
function talforweb_scripts(){
     wp_enqueue_style( 'TFW-style', plugins_url( 'css/style.css', __FILE__ ), array(), '1.1.0' );
...
{  
    ...
    if(get_option('talforweb_show_review') == '1'){
      $writeReview = '<a href="'.get_option('talforweb_write_review_url').'" target="_blank">Write Review</a>';
    }  
?>
  <div class="tfw_container">
    <div class="tfw_logoimg"><img src="<?php echo plugin_dir_url(__FILE__); ?>images/review.png" /><?php echo $writeReview; ?></div>
      <?php
